<template v-scroll-to="'.contri'">
  <div class='elenaProjects'>
    <div class='contri' v-for='author3Project in author3Projects'>
      <h1>Projects</h1>
    ...
    <transition name="modal">
      <div v-if='modal' class='project-modal'>
        ...
        <div class='all-projects'>
          <div class='all-project' v-for='author3Project in author3Projects[0]'>
            <img v-bind:src='author3Project.covers[404]'></img>
            <h4>{{author3Project.name}}</h4>
            <div class='over-stat'>
                <span class="glyphicon glyphicon-eye-open"><p>{{author3Project.stats.views}}</p></span>
                
                <span class="glyphicon glyphicon-user"><p>{{author3Project.stats.appreciations}}</p></span>
                
                <span class="	glyphicon glyphicon-thumbs-up">  <p>{{author3Project.stats.comments}}</p></span>
               
            </div>
            <a :href='author3Project.url' target='_blank'>
              <button>View Project</button>
            </a>
          </div>
        </div>
  ...
  data() {
    return {
      ...
      options: {
        ...
        xAxis: {
          categories: ['Online tutoring: Twosigmas','Internet Banking Platform: TsesnaBank', 'Corporate Website: LitsLink', 'Digital Product Website: Fashwel', 'Medical app design: Medirect', 'Chef booking Application & Website design']
        },
        yAxis: {
          min: 1000,
          max: 65000,
        ...
        series: [
          {
            name: 'Views',
            data: [16584, 23822,62988, 19051, 15936,24672]
          },
          {
            name: 'Likes',
            data: [44, 47,132, 66, 102, 189]
          }
        ]
      }
    }
  ...
  methods: {
    ...
    updateSource: function(source) {
      this.$http.jsonp('https://api.behance.net/v2/users/126487/projects?api_key=IryTnzmJFPkXW4oKRd2kQSaYTanjKD7c')
        .then(response => {
          this.author3Projects = []
          this.author3Projects.push((response.body.projects).filter(function(item) {
