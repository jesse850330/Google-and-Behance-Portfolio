<template v-scroll-to="'.contri'">
  <div class='dannyProjects'>
    <div class='contri' v-for='author4Project in author4Projects'>
      <h1>Projects</h1>
    ...
    <transition name="modal">
      <div v-if='modal' class='project-modal'>
        ...
        <div class='all-projects'>
          <div class='all-project' v-for='author4Project in author4Projects[0]'>
            <img v-bind:src='author4Project.covers[404]'></img>
            <h4>{{author4Project.name}}</h4>
            <div class='over-stat'>
                <span class="glyphicon glyphicon-eye-open"><p>{{author4Project.stats.views}}</p></span>
                
                <span class="glyphicon glyphicon-user"><p>{{author4Project.stats.appreciations}}</p></span>
                
                <span class="	glyphicon glyphicon-thumbs-up">  <p>{{author4Project.stats.comments}}</p></span>
               
            </div>
            <a :href='author4Project.url' target='_blank'>
              <button>View Project</button>
            </a>
          </div>
        </div>
  ...
  data() {
    return {
      ...
      options: {
        ...
        xAxis: {
          categories: ['Ogilvy & Mather New Zealand Office' ,'Office Stationery for Women', 'Wine Navigator', 'World Masters Games Office', 'Blood. Sweat. Cheers.', 'The Tannery Cafē.']
        },
        yAxis: {
          min: 1000,
          max: 10000,
        ...
        series: [
          {
            name: 'Views',
            data: [9822, 5339,9675, 1081, 9678,9678]
          },
          {
            name: 'Likes',
            data: [47, 32,47, 25, 44, 44]
          }
        ]
      }
    }
  ...
  methods: {
    ...
    updateSource: function(source) {
      this.$http.jsonp('https://api.behance.net/v2/users/16238999/projects?api_key=IryTnzmJFPkXW4oKRd2kQSaYTanjKD7c')
        .then(response => {
          this.author4Projects = []
          this.author4Projects.push((response.body.projects).filter(function(item) {
